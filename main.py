...
f_bodies = open('data/article_body_texts.csv', 'r', encoding='utf-8')
...
f_trainstances = open('data/train_stances.csv', 'r', encoding='utf-8')
csv_stances = csv.DictReader(f_trainstances)
for row in csv_stances:
    body = bodies[int(row['Body ID'])]
    train_list.append((row['Headline'], body, row['Stance']))
train_all = np.array(train_list)



f_valstances = open('data/validation_stances.csv', 'r', encoding='utf-8')
csv_stances = csv.DictReader(f_valstances)
for row in csv_stances:
    body = bodies[int(row['Body ID'])]
    val_list.append((row['Headline'], body, row['Stance']))
...
df = pd.read_csv('data/test_stances.csv')
df['Stance'] = l
print(df.head())
